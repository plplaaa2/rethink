...
구독 후 기기는 전체 상태 JSON을 주기적으로 또는 상태가 바뀔 때 전송한다. `Mon/Stop`을 보내면 전송이
중단된다. 확인된 상태 필드는 다음과 같다.
...
`OpMode`, `AirPollution`, `TotalAirPollution`, `DiagCode`, 예약 상태의 세부 의미와 스마트 진단용 별도
프로토콜은 아직 확인되지 않아 노출하지 않는다. 일반 모니터링 상태에는 팬 RPM이 포함되지 않는다.
